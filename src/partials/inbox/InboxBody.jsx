...
function InboxBody({
  ...
  const mails = [
    ...
    },
    {
      ...
      message: 'lorem ipsum lorem ipsum lorem insum lorem ipsum',
    },
    {
      id: '2',
      open: true,
    ...
    message: "lorem ipsum lorem ipsum",
	  ...
	  const handleSubmit = (e) => {
		...
		const addUsers = async () => {
		  const contactRef = doc(collection(db, "Chats"));
        ...
        <div className="flex items-center justify-between bg-white border-t border-slate-200 px-4 sm:px-6 md:px-5 h-16">
          ...
          <form className="grow flex">
            <div className="grow mr-3">
              ...
              <input onChange={e => setInput(e.target.value)} id="message-input" className="form-input w-full bg-slate-100 border-transparent focus:bg-white focus:border-slate-300" type="text" placeholder="Aa" />
            </div>
            <button type="submit"  onClick={handleSubmit} className="btn bg-indigo-500 hover:bg-indigo-600 text-white whitespace-nowrap">Send -&gt;</button>
          </form>
        </div>
      </div>
